...
set.seed(1020)
amostra <- round(sample(dap$diametro, 30), 2)
...
(disponível em \url{goo.gl/WPJA9G}) e faça análises descritivas para as
variáveis diâmetro à altura do peito (\texttt{diametro}) e altura das
árvores (\texttt{altura}). Os dados referem-se à amostra completa do
exercício 1.
