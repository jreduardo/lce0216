...
\item Represente o evento A = ``Pelo menos uma ave é frugívora'';
...
\begin{enumerate}[label=\alph*), labelindent=5mm]
\item $\Pr(\text{A} \cup \text{C})$
\item $\Pr(\text{B} \cup \text{E})$
\item $\Pr(\text{D} \cup \text{F})$
\item $\Pr(\text{C} \cup \text{G})$
\item $\Pr(\text{B} \cup \text{H})$
\item $\Pr(\text{C} \cup \text{D} \cup \text{E})$
...
\item A fecundidade é independente da raça? Justifique através da
  definição de independência de eventos.
...
o pacote tenha vindo da máquina A?
\vspace{1cm}
...
doente, se o técnico indicou que ela possui a doença?
